...
def run_command(cmd):
    ...
    return process.stdout.strip()  # Gabungkan stdout & stderr
...
@bot.on(events.CallbackQuery(data=b'member-vle'))
async def member_vle(event):
    async def member_vle_(event):
        cmd = 'bot-member-vme'.strip()
        try:
            z = run_command(cmd)
            await event.respond(f"""
{z}

**Shows All User Xray Actived**
**» 🤖@newbie_store24**
""", buttons=[[Button.inline("‹ Main Menu ›", "menu")]], parse_mode="markdown")
        except Exception as e:
            await event.respond(f"❌ Error: {str(e)}", parse_mode="markdown")

    sender = await event.get_sender()
    if valid(str(sender.id)) == "true":
        await member_vle_(event)
    else:
        await event.answer("Access Denied", alert=True)
...
async def vless(event):
	async def vless_(event):
		inline = [
[Button.inline("‹ MEMBER XRAY ›","member-vle")],
